...
def adjust_dates_to_start_of_month(df):
    """
        adjust typically daily series to start of month

        :param df: the input dataframe is adjusted in place
        :return: no returns
    """
...
class DataImporter:
    ...
    def get_data_from_quandle(self, fs_id):
        url_str = self.get_url(fs_id)
        df = quandl.get(url_str)
        df = df.rename(columns={'Value': fs_id})
        return df

    def get_data_from_multpl_website(self, fs_id):
        headers = {
            'User-Agent': (
    ...
    def get_data_from_fred(self, fs_id):
        url_str = self.get_url(fs_id)
        series = frd.get_series(url_str)
        series.name = fs_id
        df = series.to_frame(name=fs_id)
        df.index.name = 'Date'
        df = df.interpolate(method='cubicspline')
        return df

    def get_data_from_yahoo(self, fs_id):
        'for yahoo the url is the ticker'
        df = pd.DataFrame()

        # for yahoo identified series the url is the ticker
        ticker = self.get_url(fs_id)
        all_data = yf.Ticker(ticker)
        hist = all_data.history(period='max', interval='1d', auto_adjust=False)
        y_type = self.get_yahoo_type(fs_id)
        if y_type == MyData.close:
            y_series = hist[MyData.close]
        elif y_type == MyData.div:
            y_series = hist[MyData.div]
        elif y_type == MyData.cap:
            y_series = hist[MyData.cap]
        elif y_type == MyData.split:
            y_series = hist[MyData.split]
        elif y_type == MyData.close_adjusted:
            y_series = all_data.history(period='max', interval='1d', auto_adjust=True)[MyData.close]
        df = pd.DataFrame(y_series.rename(fs_id))
    ...
    def compute_series(self, fs_id):
        ...
        if fs_id == MyData.yahoo_spy_div_reinvest:
            prereq = [MyData.yahoo_spy_close, MyData.yahoo_spy_cap, MyData.yahoo_spy_div]

            # ensure that cap gains are 0.0
            # TODO

            # compute total return using np_arrays
            df = self.get_selected_series_as_df(prereq)
            np_close = df[MyData.yahoo_spy_close].astype(float).to_numpy()
            np_div = df[MyData.yahoo_spy_div].astype(float).to_numpy()
            np_tot_return = NumUtilities.total_return(np_close, np_div)
            # print(np_tot_return)
            df = pd.DataFrame(data=[df.index.values, np_tot_return],
                              index=['Date', MyData.yahoo_spy_div_reinvest]).T
            df.set_index('Date', inplace=True)
            print(df)
            return df
    ...
    def adjust_sequence(self, fs_id, df):
        ...
        if fs_id == MyData.sp500_pe_ratio_month:
            ...
            df1 = adjust_dates_to_start_of_month(df)
        elif fs_id == MyData.mult_trate_10_year:
            df1 = adjust_dates_to_start_of_month(df)
        else:
            df1 = df
    ...
    def get_yahoo_type(self, fs_id):
        if self.get_url_type(fs_id) != MyData.yahoo:
            return None
        fs_type = fs_id.split('_')[2]
        return fs_type
